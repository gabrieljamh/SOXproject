...
class JSONTransformer(QWidget):
    ...
    def initUI(self):
        ...
        saveButton = QPushButton("Export TavernAI JSON")
    ...
    def transformJSONAndSave(self):
        if hasattr(self, 'inputJson'):
            output_json = {
            "name": self.inputJson["name"],
            "description": self.inputJson["backstory"],  # Assuming backstory is the relevant field for description
            "personality": self.inputJson["definition"],
            "scenario": self.inputJson["default_scenario"],
            "first_mes": self.inputJson["greeting"],  # Assuming greeting can be used as an alternative
            "mes_example": self.inputJson["samples"],
            "creator_notes": self.inputJson["bio"],  # Assuming bio can be used for creator notes
            "system_prompt": "",
            "post_history_instructions": "",
            "tags": self.inputJson["social_tags"],
            "creator": self.inputJson["slug"],
